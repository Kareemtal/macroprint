...
import type { Recipe, RecipeIngredient, NutrientsPer100g } from '@/lib/types'
import {
    ArrowLeft,
    Plus,
    Trash2,
...
export function RecipeForm({ initialData, onSubmit, isSubmitting }: RecipeFormProps) {
    ...
    const [servingsPerBatch, setServingsPerBatch] = useState(initialData?.servingsPerBatch || 4)
    const [servingSizeText, setServingSizeText] = useState(initialData?.servingSizeText || '1 serving')
    const [servingSizeGrams, setServingSizeGrams] = useState<number | null>(initialData?.servingSizeGrams || null)

    const [ingredients, setIngredients] = useState<IngredientLine[]>([])

    // Initialize ingredients from initialData or default
    useEffect(() => {
        if (initialData?.ingredients) {
            setIngredients(
                initialData.ingredients.map((ing) => ({
                    ...
                    nutrients: ing.nutrientSnapshotPer100g,
                    isSearching: false,
                    searchResults: [],
                    showResults: false,
                    allergens: ing.allergensDetected,
                    providerFoodId: ing.providerFoodId,
                }))
            )
        } else {
            // Only set default if empty (initial load for new recipe)
            setIngredients([
                {
                    id: generateId(),
                    name: '',
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-muted-foreground">
                        {initialData ? 'Update recipe details and ingredients' : 'Add ingredients and calculate nutrition facts'}
                    </p>
                </div>
